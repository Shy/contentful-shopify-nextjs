export default class ShopifyApi {
  static async getCheckOutURL(variantId) {
    ...
    const fetchUrl = `https://${process.env.NEXT_PUBLIC_SHOPIFY_STORE_NAME}.myshopify.com/api/2021-07/graphql.json`;
    const fetchOptions = {
      method: "POST",
      headers: {
  ...
  static async query(query, api_key) {
    const fetchUrl = `https://${process.env.NEXT_PUBLIC_SHOPIFY_STORE_NAME}.myshopify.com/admin/api/2021-04/graphql.json`;
    const fetchOptions = {
      method: "POST",
      headers: {
